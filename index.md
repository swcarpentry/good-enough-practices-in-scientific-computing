...
2.  *Create the data you wish to see in the world*. Create
...
3.  *Create analysis-friendly data*: Analysis can be much
...
5.  *Anticipate the need to use multiple tables*: Raw data,
...
6.  *Submit data to a reputable DOI-issuing repository so that
    others can access and cite it.* Your data is as much a
...
1.  *Place a brief explanatory comment at the start of every
    program*, no matter how short it is. That comment should
...
3.  *Be ruthless about eliminating duplication*. Write and
    ...
    libraries that do what you need* before writing new code
    yourself, and *test libraries before relying on them*.

4.  *Give functions and variables meaningful names*, both
...
5.  *Make dependencies and requirements explicit.* This is
...
1.  *Create an overview of your project.* Have a short file
...
2.  *Create a shared public “to-do” list*. This can be a
...
3.  *Make the license explicit.* Have a `LICENSE` file in
...
4.  *Make the project citable* by including a `CITATION`
...
1.  *Put each project in its own directory, which is named after
    the project.* Like deciding when a chunk of code should be
...
2.  *Put text documents associated with the project in the `doc`
    directory.* This includes files for manuscripts,
...
3.  *Put raw data and metadata in a `data` directory, and files
    ...
    directory*, where “generated files” includes intermediate
...
4.  *Put project source code in the `src` directory.* `src`
...
5.  *Put external scripts, or compiled programs in the `bin`
    directory*. `bin` contains scripts that are brought in from
...
3.  *Share changes frequently.* Everyone working on the
...
1.  *Store each project in a folder that is mirrored off the
    researcher’s working machine* by a system such as Dropbox,
...
1.  *Write manuscripts using online tools with rich formatting,
    change tracking, and reference management*, such as Google
...
2.  *Include a `PUBLICATIONS` file in the project’s `doc`
    directory* with metadata about each online manuscript (e.g.,
