...
\cite{wilson2014}. It was well received, but many novices found its
...
\item
  ...
  download the entire DNA sequence database GenBank).
...
\item
  ...
  proprietary formats: they will likely last longer, and will
...
\item
  ...
  \begin{framed}
    ...
    \withurl{OpenRefine}{http://openrefine.org/} \fixme{LN}{url from frame does not appear in footnote}
...
\item
  ...
  itself. For example, if each of wells in the 8 rows by 12 columns 
  of a microtitre plate is used to
  study a specific gene knockout, the prepared raw data will have a
  \texttt{well} variable taking on values like ``A1'' and ``G12'' 
  (the column and row labels), 
  but the plate reader cannot possibly know what's in each well. That
...
\item
  ...
  minus two)\fixme{LN}{add reference?} items at once. If we are to understand what our software
...
\item
  ...
  This applies with even greater force to using software libraries. The easiest
...
\item
  ...
  counter variable in a loop \texttt{i} or \texttt{j}, the main grid\fixme{LN}{feels like jargon, is there an easier term?}
...
\item
  ...
  \begin{framed}
    ...
    controlled by an external configuration file.\fixme{LN}{Can this be explained better? I didn't really understand how this works}
  \end{framed}
...
\item
  ...
  easy for people to credit.\fixme{LN}{add examples, e.g. zenodo and figshare?}
...
\end{verbatim}
\caption{Example CITATION File with the information in two forms, as a regular citation and in BiBTeX format\fixme{LN}{explain BiBTex?}}
\label{fig:citation}
\end{figure}
