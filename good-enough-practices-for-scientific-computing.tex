...
researcher can and should consider adopting.  These recommendations synthesize
...
much longer than necessary and researchers are limited in how effectively
and they can work with software and data. Computing workflows need to
follow the same practices as with lab projects and notebooks, with organized data,
documented steps and the project structured for reproducibility, but 
...
That paper provided 'next steps' for people already working in
scientific computing. However, as computing has become an
essential part of science for all researchers, there is a larger
group of people newer to scientific computing, and the question
then becomes 'where to start?'.

This paper focuses on these first, accessible skills and perspectives - 
the 'good enough' practices - for scientific computing, 
a minimum set of tools and
techniques that we believe every researcher can and should consider adopting. It
...
handful of collaborators on projects lasting a few days to a several 
months. A practice is included in our list if large numbers of researchers 
use it, and large numbers of people are \emph{still} using it months
...
\item
  ...
  making local copies of large, stable databases.
  In that case, record the exact procedure used to obtain the raw data, as well
  as any other pertinent information, such as an official version number or the date of download. 
  
  \practice{1b}{Ensure that raw data is backed up in more than one location.} If external hard drives are used, 
  ideally they are stored off-site of the original location. Universities often have their own data
  storage solutions, so it is worthwhile to consult with your local Information Technology (IT) or library groups.
  Alternatively cloud computing resources, like Amazon
  elastic storage are reasonably priced and reliable.  For large data sets, 
  where storage and transfer can be expensive and time-consuming, you may need to 
  use incremental backup or specialized storage systems, and people in your local 
  IT group or library can often provide advice and assistance on options
  at your university or organization as well.
...
\item
  ...
  codes for missing data, such as ``-99'', with \texttt{NA},
...
\item
  ...
  to your analysis as statistical modeling and inference. If you do not document this
  ...
  data arrives. You can also re-use data preparation steps in the future for related
  projects. For very large data sets, data preparation may also include writing and saving scripts to obtain 
  the data or subsets of the data from remote storage. 

  Some data cleaning tools, such as \withurl{OpenRefine}{http://www.openrefine.org}, 
  % check OpenRefine URL
  provide a graphical user interface, but
  also automatically keep track of each step in the process. When tools like these or
  scripting is not feasible, it's important to clearly document every manual action
...
\item
  \practice{1e}{Anticipate the need to use multiple tables, and use unique identifiers}: Raw data, even if tidy, is not
...
true when working with large data sets, where storage and transfer 
of the entire data set is not trivial or inexpensive. 
...
single-purpose functions with clearly-defined inputs and outputs. Much 
has been written on this topic (refs!), and this section focuses on practices 
that best balance ease of use with benefit for you and 
collaborators.
...
\item
  ...
  one page (about 60 lines) long. Functions are sections within a program
  that do one particular thing. The syntax varies depending on programming
  language, but generally you name the function, list the parameters, or 
  information needed for that section of code, and list what information will 
  come out of the function. 
  % check definition of functions
  % WOULD BE GOOD TO HAVE EXAMPLE OF A FUNCTION
  Functions should take no more than about 5 or 6 input parameters
  and shouldn't include variables used in other parts of the program 
  (global variables).
  % I'm not sure if we should discuss global variables. I'm not sure it's
  % an easy concept to explain. Or better description of global variables here?
  The key motivation here is to fit the program
  ...
  this limit, then create programs by combining these chunks. Putting 
  code into functions also makes it easier to test and troubleshoot if
  something is going wrong. 

\item
  \practice{2c}{Be ruthless about eliminating duplication}. Write and
  re-use functions instead of copying and pasting code. You can 
  also use 
  data structures like lists, e.g. create \texttt{score = (1, 2, 3)} rather than lots of variables
  like \texttt{score1 = 1}, \texttt{score2 = 2}, \texttt{score3 = 3}, etc.

  Also look for well-maintained libraries that already do what you're trying to do. All
  programming languages have libraries that you can import and 
  use in your code. This is code that people have already written and made available
  for distribution that have a particular function. For instances there are libraries for statistics,
  modeling, mapping and many more. Many languages catalog the libraries in a centralized
  source, for instance R has CRAN, Python has ... and Java has ...
  % is this all correct?
  So \practice{2d}{always search for well-maintained
  software libraries that do what you need} before writing new code yourself,
  but \practice{2e}{test libraries before relying on them}.
...
\item
  ...
  counter variable in a loop \texttt{i} or \texttt{j}, things that are re-used often, 
  such as the major data
  structures in a program should \emph{not} have one-letter
  names.  Remember to follow each language's conventions for names,
  such as \texttt{net\_charge} for Python and \texttt{NetCharge} for
  Java.
...
\item
  ...
  the first thing users and collaborators on your project will look at, so 
 make it explicit how you want people to engage with the project. If you
 are looking for more contributors, make it
  explicit that you welcome contributors and point them to the license (more below)
...
\item
  \practice{3b}{Create a shared ``to-do'' list}.  This can be a
...
\item
  ...
  decisions about (and publicize where appropriate) how members 
  of the project will communicate with
  each other and with externals users / collaborators. This includes
  the location and technology for email lists, chat channels, voice / 
  video conferencing, documentation, and meeting notes, as well as 
  which of these channels will be public or private. 
  
\item
  \practice{3d}{Make the license explicit.}  Have a \texttt{LICENSE} file
...
\item
  ...
  its own. Projects do often require their own organizational model, but the
  below are recommendations on how you can structure data, code, analysis
  outputs and other files. The important concept is that is useful to 
  organize the project by the types of files and that consistency helps you
  effectively find and use things later. 
...
\item
  ...
  \texttt{data} directory might require subdirectories to organize 
  raw data based on time, method of collection, or other 
  metadata most relevant to your analysis. 
...
\item
  ...
  the results.
    ...
    scripts that is a script that combines all the analysis steps that
    executes the entire project from start to finish, with 
    particular
    parameters and data input/output commands. A controller script for
...
While these version control tools make keeping track of changes easier and
automatically integrates it into the workflow, they can have a steep learning
curve. So, we provide two sets of recommendations, \emph{1} a
systematic manual approach for managing changes and \emph{2} version
control in its full glory, and you can use the first while working towards the 
second, or just jump in to version control. 
...
\item
  \practice{5e}{Store each project in a folder that is mirrored off the
    researcher's working machine} using a system such as \withurl{Dropbox}{http://dropbox.com}
    or a remote version control repository such as \withurl{GitHub}{http://github.com}.
...
\end{verbatim}
  ...
  \begin{quote}
    ...
    \$5. Provided large data files are kept out of the backed-up
...
approach, and the one we use in our projects, don't just
...
free and open source. \withurl{Bitbucket}{https://bitbucket.org/} provides
...
great joys of version control. Unfortunately, Microsoft\textregistered Office files
(like the \texttt{.docx} files used by Word) or other binary files,
e.g., PDFs, can be stored in a version control system, but it is not
...
document, plain text in email, or a mix of all three. This allows co-authors
to use familiar tools, but results in
...
editor file format (e.g., Microsoft\textregistered Word \texttt{.docx} or
LibreOffice \texttt{.odt}) after major changes, then downloaded and
...
% WE'VE ALREADY MADE THE DECISION IN ANOTHER SECTION
% TO HAVE TWO SETS OF RECOMMENDATIONS. I DON'T
% THINK WE NEED THIS DESCRIPTION HERE. 
% It's a different style than the rest of the paper, and even though
% we're describing Google docs as 'good enough' it comes 
% across as being 'less than because you're not good enough to
% use these tools', which is not what we're trying to say.
% 
...
Collaborative editing in something like Google Docs if often therefore an 
effective approach. This solution meets the requirements that we've outlined above, although 
you do lose some benefits of text-based formats (notably, being able to use 
non-proprietary tools for editing, sharing, and viewing and being amenable to  
version control). We still recommend \emph{against} traditional desktop tools like
LibreOffice and Microsoft\textregistered Word because they make collaboration more
...
CSV, JSON, YAML, XML, or HDF5\footnote{We recommend against more
innovative formats in deference to an old saying: ``What's oldest lasts longest.''}.
...
coverage checking and continuous integration is more likely to be overwhelming
to newcomers rather than aid them.
...
At a local level,
PIs can have the most impact, requiring that the research their lab
produces follow these recommendations. Even if a PI doesn't have
a background in computation, they can require that students show
and share their code in lab meetings and with lab mates, that data
is available and accessible to all in the lab and that computational
methods sections are comprehensive. Best practices can include
things like paired-programming and having other lab members replicate
workflows.\ref{computing_workflows_for_biologists}. PIs can also value the time
it takes to do these things effectively and provide opportunities for
training. While it seems challenging to take time away from other
research activities, like coding itself, the time spent on the initial efforts
will pay off with more efficient and likely more innovative research.

Universities can provide support for advice and infrastructure. This is
most commonly provided by IT or High Performance Computing groups, 
and an often under appreciated resource, the libraries. Librarians have
thought about and worked with data and provenance even before 
these computational challenges and increasingly
have dedicated data librarians on staff who have an explicit service role.
Many campuses also have self-organizing student groups, as students
seek opportunities to engage with and learn from each other around 
these skills. Universities would do well to support these groups, as 
student peers are often some of the most knowledgable and they 
help grow communities of practice. Universities can also provide 
opportunities for training on these practical skills. Many resources are
available already. They would do well to make use of existing training
programs and resources, and even contribute to them, as the training 
challenges are shared by all universities, and there is not a need to replicate 
efforts. Additionally, where courses are not yet available, they can provide
access to short courses and workshops, where again materials and often 
instructors are already provided, so they can more quickly provide training. 
In our experience with Software and Data Carpentry, our two-day workshops
at universities are almost always full within a day or two of being announced,
with wait lists as long or longer than the number of attendees. It is clear that there
is extensive demand for training and unmet needs. 

Funding agencies are challenged to meet many competing needs in research. 
Many agencies are now requiring sections on data management plans and 
education and outreach activities. We need to align these requirements with
requests for funding for these activities. The true cost of implementing these
plans includes not only infrastructure, but initially increased personnel time
and resources for training. These costs will come down as researchers learn
and more effectively implement these practices, but now is the time to invest
in these activities for longer-term gains, as science will only become more
data-intensive. 

Such investment from PIs, universities, funding agencies and others
will improve confidence in the results of
...
\item Manuscripts
  ...
  % \item Include any tools needed to compile manuscripts in the project folder.
  \end{enumerate}
\end{enumerate}
}
