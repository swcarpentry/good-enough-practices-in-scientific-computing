...
their future self. (As the joke goes, your past self doesn't answer
email{\ldots}) If researchers don't see those benefits quickly enough
to pay for the slowdown that inevitably occurs when adopting a new
tool or practice, they will almost certainly switch back to their old
way of doing things. As we discuss in
...
FIXME: add citations of papers or projects that follow these rules.
...
\item
  ...
  \end{itemize}
\item
  \emph{Normalization} (the process of making data adhere to the
  recommendations in the preceding point) is treated as a processing
  step
...
The first recommendation is that \emph{every analysis step should be
  represented textually} (complete with parameter values). It isn't
...
might be organized following these recommendations. The root directory contains a
